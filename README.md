...
`v0.1.2 Customer Proof` is prepared in [RELEASE_NOTES.md](RELEASE_NOTES.md)
for the next GitHub Release. It packages buyer-specific evidence examples,
policy tuning, and the customer-shaped regression suite into a cleaner public
proof story.
...
plain-English `summary.md`, manifest, saved evidence packs, and verification
reports. Public visitors can run the sandbox, but they cannot read or export
saved evidence.
...
`summary.md`, `evidence-reader.md`, `manifest.json`,
`evidence/<check_id>.evidence.json`, and
`verification/<check_id>.verification.json`. The manifest includes bundle-level
counts, an executive verdict, the reader path, generated file paths, policy
profiles, and newest/oldest check timestamps. If no evidence exists, the
endpoint still returns a valid zip with an empty manifest and readable empty
summary/reader files.
...
`samples/policy-tuning-suite.json` adds ten near-miss checks based on the public
evidence examples: regulated approval wording, read-only agent boundaries, and
code-review version/auth summaries. It proves Sentinel catches subtle drift
without weakening safe paraphrase behavior.
...
- Publish `v0.1.2 Customer Proof` after confirming latest `main` stays green.
