...
class GuardrailTests(unittest.TestCase):
    ...
    def test_public_demo_admin_can_export_evidence_bundle(self):
        ...
        with zipfile.ZipFile(io.BytesIO(bundle)) as archive:
            ...
            reader_md = archive.read("evidence-reader.md").decode("utf-8")
        ...
        self.assertIn("evidence-reader.md", names)
        self.assertIn("Sentinel Evidence Bundle", summary_md)
        self.assertIn("Executive verdict: **Verified release-gate bundle**", summary_md)
        self.assertIn("Total audits: 5", summary_md)
        self.assertIn("What This Proves", summary_md)
        self.assertIn("What This Does Not Prove", summary_md)
        self.assertIn("Sentinel Evidence Reader", reader_md)
        self.assertIn("Reference-Bound Boundary", reader_md)
        self.assertIn("summary.md", manifest["generated_files"])
        self.assertIn("evidence-reader.md", manifest["generated_files"])
        ...
        self.assertTrue(all(f"evidence/{case['check_id']}.evidence.json" in manifest["generated_files"] for case in report["cases"]))
        self.assertTrue(all(f"verification/{case['check_id']}.verification.json" in manifest["generated_files"] for case in report["cases"]))
    ...
    def test_admin_evidence_bundle_empty_manifest(self):
        ...
        self.assertIn("application/zip", bundle_headers.get("Content-Type"))
        with zipfile.ZipFile(io.BytesIO(bundle)) as archive:
            self.assertEqual(set(archive.namelist()), {"manifest.json", "summary.md", "evidence-reader.md"})
            manifest = json.loads(archive.read("manifest.json").decode("utf-8"))
            summary_md = archive.read("summary.md").decode("utf-8")
            reader_md = archive.read("evidence-reader.md").decode("utf-8")
        ...
        self.assertEqual(manifest["reader_path"], "evidence-reader.md")
        self.assertEqual(manifest["verdict"], "No evidence exported")
        self.assertEqual(manifest["generated_files"], ["summary.md", "evidence-reader.md", "manifest.json"])
        ...
        self.assertIn("Executive verdict: **No evidence exported**", summary_md)
        self.assertIn("No saved evidence packs were available", summary_md)
        self.assertIn("No evidence was exported", reader_md)
    ...
    def test_customer_shaped_regression_suite_passes(self):
        suite = json.loads((ROOT / "samples" / "customer-shaped-regression-suite.json").read_text(encoding="utf-8"))
        report = run_suite(suite, save_evidence=False)
    ...
    def test_public_evidence_examples_are_complete_and_verified(self):
        examples = [
            ("support-assistant", 5, 1, 4, {"agent_tool", "regulated", "research", "support"}),
        ...
        for example_name, count, emitted, blocked, policy_profiles in examples:
            with self.subTest(example=example_name):
                self._assert_public_evidence_example(
                    ROOT / "docs" / "evidence-examples" / example_name,
                    count=count,
                    emitted=emitted,
                    blocked=blocked,
    ...
    def _assert_public_evidence_example(
        self,
        example_dir: Path,
        *,
        count: int,
        ...
        self.assertEqual(manifest["summary"]["total_audits"], count)
        self.assertEqual(manifest["summary"]["emitted"], emitted)
        self.assertEqual(manifest["summary"]["blocked"], blocked)
        self.assertEqual(manifest["summary"]["verified"], count)
        self.assertEqual(manifest["summary"]["failed_verification"], 0)
        self.assertEqual(set(manifest["summary"]["policy_profiles"]), policy_profiles)
        self.assertEqual(suite_report["status"], "PASS")
        self.assertEqual(suite_report["summary"]["case_count"], count)
        self.assertEqual(suite_report["summary"]["passed"], count)
        ...
        self.assertEqual(len(evidence_files), count)
        self.assertEqual(len(verification_files), count)
